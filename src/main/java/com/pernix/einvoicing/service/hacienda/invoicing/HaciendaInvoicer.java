...
public class HaciendaInvoicer implements InvoicerService {
	...
	public String save(Invoice invoice) {
		...
		// Se deberá brindar una cabecera (header) "Authorization" con el valor del
		// access token obtenido anteriormente.
		request.header("Authorization", "Bearer " + HaciendaOAuth2Service.getAccessToken());

		// Se envía un POST. con los datos del documento que deseamos registrar, observe
		// que colocamos como
		...
		case 201:
			// Éste código de retorno se da por recibido a la plataforma el documento.
			// Posteriormente
			// debe validarse su estado de aceptación o rechazo. Es importante hacer notar
			// que se
			// regresa un header "Location" que corresponde a un URL. donde se puede validar
			// el
			...
			break;
		case 400:
			// Se da si se detecta un error en las validaciones, por ejemplo: si intento
			// enviar más de una
	...
	private Invoice signXML(Invoice invoice) {
		...
		invoice.setXmlInvoice(signer.sign(certificatePath, certificatePass, invoice.getXmlInvoice()));
		return invoice;
	}
